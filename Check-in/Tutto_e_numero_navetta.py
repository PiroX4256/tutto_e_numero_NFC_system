    ...
    if cursor.rowcount == 0:
        ...
        if checkreg == 1:
            ...
            if cursor.rowcount == 0:
                ...
                client.update()
                status.configure(text="ERRORE: Braccialetto gia" + u'\u0300' +" registrato", background="#d34343")
                status.update()
                sleep(2)
    db.close()
    clear_textbox()
...
app = Tk()
app.title("Tutto e" + u'\u0300' +" Numero - Navetta")
app.geometry("900x900+350+50")

entrypass = StringVar()

title = Label (app, text="Tutto e' Numero 2019 | Check-in", font=("Bahnschrift", 45), fg="#2077bf").pack(padx = 5, pady =0) #Title

row1 = Label(app, text = "NAVETTA", font=("Bahnschrift", 40), fg="#d34343").pack(padx = 20, pady = 0) #First text row

row2 = Label(app, text = "Inserire ID Navetta", font=("Bahnschrift", 30)).pack(padx = 30, pady = 30) #Second text row
